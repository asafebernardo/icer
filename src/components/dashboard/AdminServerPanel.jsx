...
function MiniRow({ label, value, detail }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-border/50 py-2.5 last:border-0">
      ...
      <p className="shrink-0 text-right text-sm tabular-nums text-foreground">{value}</p>
    </div>
  );
}
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard
              ...
              status={`${info.mongodb?.stats?.collections ?? "—"} coleções`}
            />
            <StatCard
              icon={Server}
              label="Processo"
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                <div className="rounded-xl border border-border bg-muted/20 p-4 lg:col-span-2">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-foreground">Disco do servidor</p>
                  ...
                  <ProgressBar value={disk?.used_pct} tone={disk?.used_pct > 85 ? "danger" : "accent"} />
                </div>
                <div className="rounded-xl border border-border bg-muted/20 p-4">
                  <p className="text-sm font-medium text-foreground">Uploads registrados</p>
                ...
                <div className="space-y-3">
                  ...
                  {siteFiles?.by_type?.length ? (
                    siteFiles.by_type.map((row) => (
                      <div key={row.type} className="rounded-xl border border-border bg-muted/20 p-3">
                        <div className="mb-2 flex items-center justify-between gap-3">
                          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-sm font-medium text-foreground">Heap Node.js</p>
                  <span className="text-sm tabular-nums text-muted-foreground">
                    {heapPct ?? "—"}%
                  </span>
                </div>
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-sm font-medium text-foreground">RAM do sistema</p>
                  <span className="text-sm tabular-nums text-muted-foreground">
                    {ramPct ?? "—"}%
                  </span>
                </div>
                <ProgressBar value={ramPct} tone={ramPct > 85 ? "danger" : "accent"} />
                <p className="mt-2 text-xs text-muted-foreground">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-dashed border-border bg-muted/30 px-4 py-3 text-xs text-muted-foreground">
