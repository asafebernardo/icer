...
/** Após fechar, remove o toast do estado (ms) — tempo para animação de saída. */
const TOAST_REMOVE_DELAY = 800;
/** Duração padrão em que o toast fica visível até fechar sozinho (ms). */
...
/** `toastId` → timeout do auto-dismiss */
const autoDismissTimers = new Map();

function clearToastAutoDismissTimer(toastId) {
...
function dispatch(action) {
  if (action.type === actionTypes.DISMISS_TOAST) {
    if (action.toastId != null) {
      clearToastAutoDismissTimer(action.toastId);
    } else {
